...
# plot the R^2
p1 = ggplot(data_long,
           aes(x=as.factor(IV_strength), y=R2_1_mean, group=IV_strength)) +
    xlab("IV strength") +
    ylab("mean of R2 for the 1st genomic modality") +
    ylim(0, 0.6) +
    geom_boxplot()
p2 = ggplot(data_long,
           aes(x=as.factor(IV_strength), y=R2_2_mean, group=IV_strength)) +
    xlab("IV strength") +
    ylab("mean of R2 for the 2nd genomic modality") +
    ylim(0, 0.6) +
    geom_boxplot()
p3 = ggplot(data_long,
           aes(x=as.factor(IV_strength), y=R2_3_mean, group=IV_strength)) +
    xlab("IV strength") +
    ylab("mean of R2 for the 3rd genomic modality") +
    ylim(0, 0.6) +
    geom_boxplot()
ggarrange(p1, p2, p3, nrow=1)
pdf(file.path(real_data_dir, "figures/simulation_R2_boxplot.pdf"), width=8.5, height=5.5)
ggarrange(p1, p2, p3, nrow=1)
dev.off()
```
...
## When the effect size varies across SNPs

Suppose the effect size of SNPs is between 0 and 2. It equals to two fold of a random variable following a beta distribution with shape1 = 5 and shape2 = 5.

```{r varying_effect_size, fig.dim = c(5, 6)}
data_all = read.csv(file.path(simulation_dir, "result_beta_dist.csv"))
...
#  "outcome type = binary_use_all_in_xQTL, r2 = ", rtwo, "\n",
#  "overlap: 0; xQTLs from same sample"))
pdf(file.path(real_data_dir, "figures/simulation_power_pval_combination_number_of_SNPs_5_beta_dist.pdf"), width=5.5, height=6)
p %+% subset(data_long_power_subset, number_of_SNPs == 5)
dev.off()
pdf(file.path(real_data_dir, "figures/simulation_power_pval_combination_number_of_SNPs_20_beta_dist.pdf"), width=5.5, height=6)
p %+% subset(data_long_power_subset, number_of_SNPs == 20) 
dev.off()
pdf(file.path(real_data_dir, "figures/simulation_type_I_error_pval_combination_number_of_SNPs_5_beta_dist.pdf"), width=3.9, height=6)
(p + 
        geom_hline(yintercept=0.05, color="red") + 
        ylab("Type I error") +
        ylim(0, 0.6) +
    ...
    subset(data_long_type_I_error_subset, number_of_SNPs == 5)
dev.off()
pdf(file.path(real_data_dir, "figures/simulation_type_I_error_pval_combination_number_of_SNPs_20_beta_dist.pdf"), width=3.9, height=6)
(p + 
        geom_hline(yintercept=0.05, color="red") +
        ylab("Type I error") +
        ylim(0, 0.6) +
...
## When genotypes reflect genetic architecture around gene *APOE*

```{r real_genotypes, fig.dim = c(5, 6)}
# NOTE: check "nsnps_mean" and "nsnps_sd" in data_all for count of IVs
data_all = read.csv(file.path(simulation_dir, "result_real_genotypes.csv"))
...
rtwo = number_of_SNPs = NA
data_long_power_subset = subset(data_long_power,
                                grepl("^MultivariableGLS$|GLS_Fisher_gamma|GLS_Cauchy|GLS_HMP|GSMR_Cauchy|GSMR_HMP|SMR_allSNPs_Cauchy|SMR_allSNPs_HMP|SMR_singleSNP_Cauchy|SMR_singleSNP_HMP", method) &
                                    grepl("0; xQTLs from same sample", overlap) &
                                    grepl("continuous", outcome)) %>%
    mutate(method = fct_relevel(method, "MultivariableGLS", "GLS_Fisher_gamma", "GLS_Cauchy", "GSMR_Cauchy", "SMR_allSNPs_Cauchy", "SMR_singleSNP_Cauchy", "GLS_HMP", "GSMR_HMP",  "SMR_allSNPs_HMP", "SMR_singleSNP_HMP")) %>%
    mutate(MR_method = recode(method, "MultivariableGLS"="GLS", "GLS_Fisher_gamma"="GLS", "GLS_Cauchy"="GLS", "GSMR_Cauchy"="GSMR", "SMR_allSNPs_Cauchy"="SMR_allSNPs", "SMR_singleSNP_Cauchy"="SMR_singleSNP", "GLS_HMP"="GLS", "GSMR_HMP"="GSMR",  "SMR_allSNPs_HMP"="SMR_allSNPs", "SMR_singleSNP_HMP"="SMR_singleSNP")) %>%
    mutate(CombineModalities = recode(method, "MultivariableGLS"="Multivariable", "GLS_Fisher_gamma"="FisherGamma", "GLS_Cauchy"="Cauchy", "GSMR_Cauchy"="Cauchy", "SMR_allSNPs_Cauchy"="Cauchy", "SMR_singleSNP_Cauchy"="Cauchy", "GLS_HMP"="HMP", "GSMR_HMP"="HMP", "SMR_allSNPs_HMP"="HMP", "SMR_singleSNP_HMP"="HMP"))
...
#  "outcome type = binary_use_all_in_xQTL, r2 = ", rtwo, "\n",
#  "overlap: 0; xQTLs from same sample"))
pdf(file.path(real_data_dir, "figures/simulation_power_pval_combination_real_genotypes.pdf"), width=5.5, height=6)
p
dev.off()
pdf(file.path(real_data_dir, "figures/simulation_type_I_error_pval_combination_real_genotypes.pdf"), width=3.9, height=6)
(p + 
        geom_hline(yintercept=0.05, color="red") + 
        ylab("Type I error") +
        ylim(0, 0.6) +
        guides(color=FALSE, size=FALSE, shape=FALSE))  %+%
    data_long_type_I_error_subset
dev.off()
```
...
# > table(result_matrix[,"number_of_SNPs_1"])
# 
#   1   2   3   4   5   6   8  11  12  13 
#  26 332 139  23   9   5   3   1   1   1 
# > table(result_matrix[,"number_of_SNPs_2"])
# 
#   1   2   3   4   5   6   7   9  11  14 
#  25 326 149  23   9   2   1   2   2   1 
# > table(result_matrix[,"number_of_SNPs_3"])
# 
#   1   2   3   4   5   6   7   9  11  12 
#  17 361 126  14  12   4   2   1   2   1 
# > table(result_matrix[,"number_of_SNPs_MVMR"])
# 
#   1   2   3   4   5   6   7   9  11  12  14 
#  29 321 149  21  11   3   2   1   1   1   1 
```
